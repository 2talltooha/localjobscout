...
from localjobscout.scrapers.stmarys import (  # noqa: E402
    extract_description_adaptive,
    extract_items_adaptive,
)

_BASE_URL = "https://www.smgh.ca/about-st-marys/careers"

_LISTING_HTML = """
<html><body>
<li class="views-row">
  <a href="/jobs/12">PSW Aide</a>
  <span class="department">Long-Term Care</span>
</li>
<li class="views-row">
  <a href="/jobs/13">Housekeeper</a>
</li>
</body></html>
"""

_LONG_BODY = (
    "A reasonably long job description body for St. Mary's General Hospital posting."
)
...
def test_extract_items_adaptive_pulls_fields() -> None:
    ...
    assert len(items) == 2
    first = items[0]
    assert first["title"] == "PSW Aide"
    assert first["href"] == "https://www.smgh.ca/jobs/12"
    assert first["department"] == "Long-Term Care"


def test_extract_items_adaptive_dedupes_via_seen_set() -> None:
    sel = Selector(content=_LISTING_HTML, adaptive=True)
    seen: set[str] = {"https://www.smgh.ca/jobs/12"}
    items = extract_items_adaptive(sel, _BASE_URL, seen)
    assert len(items) == 1
    assert items[0]["title"] == "Housekeeper"
